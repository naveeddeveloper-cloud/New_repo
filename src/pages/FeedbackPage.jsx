...
// --- MODIFIED --- : Added `...props` to pass down value, onChange, etc.
const InputWithIcon = ({ id, type, placeholder, required, icon, ...props }) => (
  <div className="relative">
    ...
    <input
      ...
      {...props} // Pass down other props like value and onChange
    />
  </div>
);
...
const FeedbackPage = () => {
    ...
    // --- ADDED --- : State for name input and its validation error
    const [name, setName] = useState('');
    const [nameError, setNameError] = useState('');

    const ratingLabels = ['Poor', 'Fair', 'Good', 'Very Good', 'Excellent'];

    // --- ADDED --- : Handler for name input changes with validation
    const handleNameChange = (e) => {
        const value = e.target.value;
        setName(value);

        // Regex to check for valid characters (letters, spaces, hyphens, apostrophes)
    ...
    const handleSubmit = (e) => {
        ...
        // --- MODIFIED --- : Added checks for name field validation
    ...
    const handleReset = () => {
        ...
        // --- ADDED --- : Reset name state on form reset
        setName('');
        setNameError('');
    }
                            ...
                            {!isSubmitted ? (
                                ...
                                >
                                    <form onSubmit={handleSubmit} className="space-y-6">
                                        {/* --- MODIFIED --- Name Input Field with Validation */}
                                        <div>
                                            <InputWithIcon
                                                id="name"
                                                type="text"
